...
function OptimizationPage() {
      ...
      try {
        ...
        if (value.generated_pdf) {
          try {
            ...
            setError('Your PDF is still saved, but its temporary access link could not be restored. Try Restore Saved PDF.')
          }
        }
      } catch {
        setError('Optimization was not found or is unavailable.')
      } finally {
        setLoading(false)
      }
    }
  ...
  async function apply() { setWorking(true); setError(''); try { const result = await applyOptimizations(optimizationId, suggestions); setData((current) => ({ ...current, status: 'applied', optimized_resume: result.optimized_resume, after_match: result.match })); setSuggestions((items) => items.map((item) => ({ ...item, status: item.status === 'pending' ? 'rejected' : item.status }))) } catch (requestError) { setError(requestError.response?.data?.detail?.message || 'Unable to apply these decisions.') } finally { setWorking(false) } }
  async function generate() { setWorking(true); setError(''); try { const result = await generateFinalResume(optimizationId); setPdf(result); setData((current) => ({ ...current, status: 'generated', generated_pdf: { filename: result.filename } })) } catch { setError('Unable to generate the final PDF.') } finally { setWorking(false) } }
  async function refresh() { setWorking(true); try { setPdf(await refreshPDFAccess(optimizationId)) } catch { setError('Unable to refresh secure PDF access.') } finally { setWorking(false) } }
